...
template<typename vec, int _dims>
class KdTreeSearchNDim {
public:
	KdTreeSearchNDim();
	virtual ~KdTreeSearchNDim();

	//______________ tree construction __________________________

	virtual void begin() ;
	virtual void add_point(vec* v) ;
	virtual void add_vertex_set(std::vector<vec>* vs) ;
	virtual void end() ;
	...
	// return the indices of the k neighbors of p. You can use these indices to get 
	// the 3D coordinates of the neighbor points 

	int find_neighbors(const vec& p, unsigned int k, 
		float sqRad,
		std::vector<int>&neighbors,
		std::vector<float>&neighborDistance) const;

	//_________________ K-nearest neighbors within given radius ____________________
	// p: the query point.
	// k: number of neighbors requested.
	// return the indices of the k neighbors of p. You can use these indices to get 
	// the 3D coordinates of the neighbor points 
	...
	ANNpointArray get_pointarray()const
	{
		return catched_points_;
	}


protected:
	std::vector<vec*>	vertices_;
	std::size_t			points_num_;
	ANNkd_tree*				tree_;

	//double **
	ANNpointArray		catched_points_;
} ;


template<typename vec, int _dims>
KdTreeSearchNDim<vec, _dims>::KdTreeSearchNDim()  {
	points_num_ = 0;
	tree_ = 0;
	catched_points_ = 0;
}
...
template<typename vec, int _dims>
void KdTreeSearchNDim<vec, _dims>::begin()  {
	vertices_.clear();
...
template<typename vec, int _dims>
void KdTreeSearchNDim<vec, _dims>::end()  {
	points_num_ = vertices_.size();
	catched_points_ = annAllocPts(int(points_num_), _dims);

	for (std::size_t i = 0; i < points_num_; ++i)
	{
		for (int j=0;j<_dims;j++)
		{
			catched_points_[i][j] = (*vertices_[i])[j];
		}
	}

	tree_ = new ANNkd_tree(catched_points_, int(points_num_), _dims);
}

template<typename vec, int _dims>
void KdTreeSearchNDim<vec, _dims>::add_point(vec* v)  {
	vertices_.push_back(v);
}

template<typename vec, int _dims>
void KdTreeSearchNDim<vec, _dims>::add_vertex_set(std::vector<vec>* vs)  {
	std::vector<vec>& points =*vs;
	for (std::size_t i = 0; i < points.size(); ++i)
		vertices_.push_back(&(points[i]));
}

template<typename vec, int _dims>
int KdTreeSearchNDim<vec, _dims>::find_neighbors(const vec& p, unsigned int k, 
												 std::vector<int>&neighbors,
												 std::vector<float>&neighborDistance) const
{
	neighbors.clear();
	...
	tree_->annkSearch(ann_p, k, closest_pts_idx, closest_pts_dists);
	neighbors.resize(k);
	neighborDistance.resize(k);
	for (unsigned int i = 0; i < k; ++i) {
	neighbors[i] = closest_pts_idx[i];
	neighborDistance[i]=closest_pts_dists[i];
	}
...
template<typename vec, int _dims>
int KdTreeSearchNDim<vec, _dims>::find_neighbors(const vec& p, unsigned int k, float radius, 
												 std::vector<int>&neighbors, 
												 std::vector<float>&neighborDistance) const
{
	if (radius<0)
	...
	//count the number of sqRad
	int neighborPointsNum=tree_->annkFRSearch(ann_p,sqRad,0);
	int maxSearchNum;
	if (k<=0)//search all neighbor points
	{
		maxSearchNum=neighborPointsNum;
	}
	else
	{
		maxSearchNum=k;
		if (maxSearchNum>neighborPointsNum)
		{
			maxSearchNum=neighborPointsNum;
		}
	}
	ANNidxArray closest_pts_idx = new ANNidx[maxSearchNum];	// near neighbor indices
	ANNdistArray closest_pts_dists = new ANNdist[maxSearchNum];	// near neighbor distances
	tree_->annkSearch(ann_p, maxSearchNum,closest_pts_idx, closest_pts_dists);

	neighbors.resize(maxSearchNum);
	neighborDistance.resize(maxSearchNum);
